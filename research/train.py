...
if __name__ == "__main__":
    ...
    if multi_prox.sum() > 0:
        ...
        log.info(
            "Group class purity: mean=%.3f  fully_pure=%.1f%%",
            pur.mean(),
            (pur == 1.0).mean() * 100,
        )
        if pur.mean() < 0.95:
    ...
    log.info(
        "Score AFTER  consensus : %.6f  (%d misses)",
        score_consensus,
        int((preds_consensus != y).sum()),
    )
    log.info(
        "Predictions changed    : %d  (corrected=%d  broken=%d)", n_changed, n_corrected, n_broken
    )
    ...
    np.save(
        str(ARTIFACTS_DIR / "threshold_biases.npy"), biases
    )  # [0.0, b1, b2] — class-0 is reference
