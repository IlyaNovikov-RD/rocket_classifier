...
make test             # uv run pytest tests/ -v (runs after artifacts exist → 111/111)
# Inference
...
- **Threshold biases** `[0.0, -0.25316455696202533, 1.2658227848101262]`: Applied as `argmax(log(proba) + biases)` to shift decision boundaries toward minority classes (class distribution: 69%/24%/7%). Exact values saved in `artifacts/threshold_biases.npy` and `training_report.json`.
